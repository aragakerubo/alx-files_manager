...
class DBClient {
    ...
    // Get files by paginated files by userId, parentId
    async getPaginatedFilesByParentId(userId, parentId, page, pageSize) {
        if (!this.db) return 0;
        try {
            const matches = {
                userId: userId,
            ...
            console.error(`Error fetching files by parentId: ${error.message}`);
            return 0;
        }
    }
}
